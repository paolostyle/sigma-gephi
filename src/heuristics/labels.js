 ...
 */
export default function labelsToDisplayFromGrid(params) {
  const { cache, camera, displayedLabels, visibleNodes, dimensions, graph } = params;
  ...
  const panning =
    cameraState.x !== previousCameraState.x || cameraState.y !== previousCameraState.y;
  const unzooming = cameraState.ratio > previousCameraState.ratio;
  const unzoomedPanning = !zooming && !unzooming && cameraState.ratio >= 1;
  ...
  const adjustedHeight = dimensions.height + cellHeight;
  const adjustedX = -cellWidth;
  const adjustedY = -cellHeight;

  const panningWidth = dimensions.width + cellWidth / 2;
  const panningHeight = dimensions.height + cellHeight / 2;
  const panningX = -(cellWidth / 2);
  const panningY = -(cellHeight / 2);
  ...
  for (let i = 0, l = visibleNodes.length; i < l; i++) {
    ...
    const yKey = Math.floor(pos.y / cellHeight);
    ...
    if (typeof grid[key] === 'undefined') {
      ...
      if (nodeData.size > currentNodeData.size) {
        ...
        const currentNodeDegree = graph.degree(currentNode);
  ...
  for (let i = 0, l = worthyLabels.length; i < l; i++) {
    ...
    const d1 = cache[n1];
    const p1 = camera.graphToViewport(dimensions, d1.x, d1.y);
    ...
    for (let j = i + 1; j < l; j++) {
      ...
      const d2 = cache[n2];
      const p2 = camera.graphToViewport(dimensions, d2.x, d2.y);
